...
for i = 1:numPackets
    ...
    % Generate packet header as a binary vector
    packetHeader = dec2bin(i, 16) - '0'; % 16-bit header for packet number
    packetHeader = bi2de(reshape(packetHeader, log2(M), []).', 'left-msb');
    
    % Prepend preamble to the packet
    packet = [barkerSequence, packetHeader.', packetData.'];
