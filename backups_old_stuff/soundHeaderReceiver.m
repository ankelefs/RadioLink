...
while i<27
    ...
    %----------------------------FRAME SYNC----------------------------------
    [rxSigFrames, packetCompletes,dataStartIdxs, extractedPacketNumbers] = extractHeaderPackets(rxSigSync, barkerSequence, M, dataLength);
    %packetComplete;
    ...
    for packetIdx = 1:length(rxSigFrames) 
        ...
        if packetComplete
            ...
            demodulatedPacketData{end+1} = rxDataDemod;
            packetNumbers(end+1) = extractedPacketNumbers(packetIdx);
        end
    end
...
% After processing all packets, sort and reorder the packets based on their numbers
[~, sortedIndices] = sort(packetNumbers);
sortedDemodulatedData = demodulatedPacketData(sortedIndices);

% Concatenate the sorted, demodulated data
allDemodulatedPackets = vertcat(sortedDemodulatedData{:});
