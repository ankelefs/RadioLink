...
if ~isempty(partialPacket)
        ...
        rxSignal_Frame = [partialPacket; inputSignal(overlapBufferLength:neededLength)];
...
barkerSymbols = pskmod(barkerSequence, modulationOrder, pi/modulationOrder, 'gray');
preambleDetectorObject = comm.PreambleDetector(barkerSymbols.', 'Threshold', preambleThresholdFactor);
preambleDetectorIndex = preambleDetectorObject(inputSignal);
...
for i = 1:length(preambleDetectorIndex)
    dataStartIndex = preambleDetectorIndex(i) + 1;
    ...
    if (dataStartIndex + informationDataSize - 1) <= length(inputSignal)
        lastDataStartIndex = dataStartIndex;
        rxSignal_Frame = inputSignal(dataStartIndex:dataStartIndex + informationDataSize -1);
        rxSignal_Frames{end+1} = rxSignal_Frame;
        packetCompletions(end+1) = true;
